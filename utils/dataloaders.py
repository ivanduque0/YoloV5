    ...
    def update(self, i, stream):
        while True:
            try:
                ...
                im = cv2.imdecode (imagenarray,-1)
                alto, ancho, _ = im.shape
                K = np.float32([[1,0,100],[0,1,100]])
                video2 = cv2.warpAffine(im, K, (ancho+200,alto+200))
                ...
                im = cv2.warpAffine(video2, K, (alto, ancho))
                im = cv2.flip(im, 0) 
                #print(f'loader: {im.shape}')
                self.imgs[i] = im
            except:
